...
2. Сервис ddg-idd-driver при старте создает файл ddg_states.json. в нем он хранит все последние состояния устройств и модель. Устройства создаются в этом файле после первого принятого сообщения от счетчика. Если при запуске сервиса файл уже существует, то будет немедленно отправлены все параметры состояния 
